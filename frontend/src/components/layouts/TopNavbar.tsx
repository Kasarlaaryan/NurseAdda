      ...
      <div className="h-full px-4 sm:px-6 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link to="/dashboard" className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-xl bg-sky-600 dark:bg-sky-500 text-white flex items-center justify-center font-bold text-lg shadow-xs">
              N+
            </div>
            <div className="hidden sm:flex flex-col">
              ...
              <span className="text-[10px] uppercase font-semibold tracking-wider text-sky-600 dark:text-sky-400 mt-0.5">
                Workforce OS
              </span>
            </div>
          </Link>
        ...
        <div className="hidden md:flex flex-1 max-w-md mx-4">
          <div className="relative w-full">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 text-xs rounded-xl bg-slate-100/80 dark:bg-slate-800/80 border border-transparent focus:border-sky-500 focus:bg-white dark:focus:bg-slate-900 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 transition-all focus:outline-none"
            />
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
            >
            ...
            {showRoleMenu && (
              <div
                className="absolute right-0 mt-2 w-64 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl p-2 z-50 animate-in fade-in zoom-in-95 duration-150"
                onClick={() => setShowRoleMenu(false)}
              >
                <div className="px-3 py-2 border-b border-slate-100 dark:border-slate-800">
                  <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Switch Persona / Role</p>
                  <p className="text-xs text-slate-500 mt-0.5">Test role-based access control</p>
                </div>
                <div className="py-1 space-y-1">
                  {(Object.keys(USER_ROLES_CONFIG) as UserRole[]).map((r) => {
                    const cfg = USER_ROLES_CONFIG[r];
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 rounded-xl text-xs flex flex-col transition-colors ${
                          isCurrent
                            ? 'bg-sky-50 dark:bg-sky-950/60 font-bold text-sky-700 dark:text-sky-300'
                            : 'hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300'
                        }`}
                      >
                        <span className="font-semibold">{cfg.label}</span>
                        <span className="text-[10px] opacity-75 font-normal truncate">{cfg.description}</span>
                      </button>
                    );
                  })}
                </div>
            ...
            {showNotifMenu && (
              ...
              >
                  ...
                  {notifications.slice(0, 5).map((n) => (
                    <div
                      ...
                      className={`p-2.5 rounded-xl border text-xs transition-colors ${
                        !n.read
                          ? 'bg-sky-50/50 dark:bg-sky-950/30 border-sky-200 dark:border-sky-900/40'
                ...
                <Link
                  ...
                  className="block text-center text-xs font-semibold text-sky-600 dark:text-sky-400 py-1.5 hover:underline border-t border-slate-100 dark:border-slate-800 mt-1"
                >
